...
class SurfaceTransformWhiteInputSpec(FSTraitedSpec):
    ...
    subject_id = traits.String(mandatory=True, argstr='--s %s', desc='subject as both source and target')
...
class MakeSurfacesInputSpec(FSTraitedSpec):
    ...
    variable_sigma = traits.String(argstr='-variablesigma %s', desc='variable_sigma')
...
class MakeSurfaces(FSCommand):
    ...
    def _format_arg(self, name, spec, value):
        ...
        if name in ['in_T2']:
            ...
            suffix = '.'.join(suffix)
            return spec.argstr % os.path.join(self.inputs.subject_id,
                                              'mri', suffix)
