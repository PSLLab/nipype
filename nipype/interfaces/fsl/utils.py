...
class Affine2RigidInputSpec(FSLCommandInputSpec):
    in_file = File(exists=True,
                   desc='input filename',
...
class Affine2RigidOutputSpec(TraitedSpec):
    out_matrix = File(exists=True, desc="rigid output matrix")

class Affine2Rigid(FSLCommand):
    _cmd = 'aff2rigid'
    input_spec = Affine2RigidInputSpec
    output_spec = Affine2RigidOutputSpec
...
class ConvertXFMInputSpec(FSLCommandInputSpec):
    ...
    fix_scale_skew = traits.Bool(argstr="-fixscaleskew", position=-3,
                                 ...
                                 desc=("use secondary matrix to fix scale and "
                                       "skew"))
    out_file = File(name_template='%s_xfm.mat', name_source=['in_file'], keep_extension=True, argstr="-omat %s", position=1,
...
class ConvertWarpInputSpec(FSLCommandInputSpec):
    ...
    jacobian_out = File(argstr='--jacobian=%s', desc='jacobian output', output_name='jacobian_out',
                        name_source=['reference'], name_template='%s_jac')
